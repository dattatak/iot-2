...
if (isset($_GET['id'])){
	if (isset($_GET['set'])){
		...
		if(empty($result)) {
			...
			$query = "INSERT INTO `" . $_GET['id'] . "` (data) SELECT '" . $_GET['set'] . "' from (select datetime,data from `" . $_GET['id'] . "` order by `key` desc limit 1 ) as1 where as1.data <> '" .$_GET['set'] . "' AND as1.datetime >= DATE_SUB(NOW(),INTERVAL 1 HOUR";
			print $query;
			$result = mysqli_query($con, $query);
			# print $result;
		}
